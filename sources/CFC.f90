program NFCA
  ...
  real(kind=r4) :: SEARCHBOX(2,2)
  integer, parameter :: TEMPERATURE=1, TIME=2, START=1, END=2
  real(kind=r4) :: MAXSURFTEMP
  integer :: mopt
  integer :: nproc
  integer :: iproc
  logical :: lroot  
  ...
  read(LU_param,*) MINSEARCHRADIUS
  read(LU_param,*) MAXSEARCHRADIUS
  ...
  read(LU_param,*) SEARCHBOX(START,TEMPERATURE)
  read(LU_param,*) SEARCHBOX(END,TEMPERATURE)
  read(LU_param,*) SEARCHBOX(START,TIME)
  read(LU_param,*) SEARCHBOX(END,TIME)
  read(LU_param,*) MAXSURFTEMP
  read(LU_param,*) nmaps
  ...
  FINALTIME = SEARCHBOX(END,TIME)
  MAXSEARCHRADIUS = MAXSEARCHRADIUS * 1000._r4
  MINSEARCHRADIUS = MINSEARCHRADIUS * 1000._r4
  ...
  do I=1, ndata
    ...
    ! Include samples within MINSEARCHRADIUS
    do J =1, ndata
      if(distances(I,J) .lt. MINSEARCHRADIUS) then
        ADD_POINTS = .TRUE.
        do L=1,KK
  ...
  do I=1,ndata
    ...
    range(1,1:(ttpoints-1))   = SEARCHBOX(START,TIME)
    range(2,1:(ttpoints-1))   = SEARCHBOX(END,TIME)
    range(1,4:(2*ttpoints-1)) = SEARCHBOX(START,TEMPERATURE)
    range(2,4:(2*ttpoints-2)) = SEARCHBOX(END,TEMPERATURE)
    range(2,(2*ttpoints-1))   = MAXSURFTEMP
    ...
    sample(I)%bestpath(1,ttpoints)       = FINALTIME
...
subroutine writemodels (nd,ntot,models,misfit,ns1,ns2,itmax, &
  ...
  write (87, 302) mfitmin, model_opt(1:(ttpoints-1)), FINALTIME, model_opt(4:(2*ttpoints-1))  
  write (87,'(80a)') ("-", I = 1, 80)
