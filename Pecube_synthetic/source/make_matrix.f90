      subroutine make_matrix (mpe,ael,bel,icon,xg,yg,zg,ug,xgp,ygp,zgp,ugp, & !VTK
                              ...
                              xming,xmaxg,yming,ymaxg,friction)
...
! this is where the FE matrices and RHS vectors are built
      ...
      double precision,dimension(:),allocatable :: x,y,z,u,xp,yp,zp,up,h,ht,dhdr,dhds,dhdt,rb !VKP
      ...
      double precision eps,rhoc,cond,heat,dynamic,dt,alpha,timep,xint,yint,zint,rint,uint !VKP
      double precision veloz,volume,volumep,epsij,velox,veloy,uvnorm,xmin,xmax,ymin,ymax,velou !VKP
      double precision xming,xmaxg,yming,ymaxg,friction,specific_heat
      double precision zmin,zmax,dx,dy,dz,tau,xcond,xmass,xcondp,xmassp,fixt
      double precision r,s,t,w,vx,vy,vz

      allocate (aelp(mpe,mpe),b(mpe,3),bp(mpe,3),rb(mpe))
      allocate (x(mpe),y(mpe),z(mpe),u(mpe),xp(mpe),yp(mpe),zp(mpe),up(mpe)) !VKP
      allocate (vv(mpe,3))
      ...
      timep=time-dt
        ...
        do i=1,mpe
        call find_velo (x(i),y(i),z(i),time,dt,fault,nfault,vv(i,1),vv(i,2),vv(i,3),xming,xmaxg,yming,ymaxg)
        enddo
        ...
        jcbip(3,3)=(jcbp(1,1)*jcbp(2,2)-jcbp(1,2)*jcbp(2,1))/volumep
          ...
          do j=1,3
            ...
            enddo
            heat=heat+abs(epsij)*(1.d8/3.d3)*friction/specific_heat
      	    enddo
          enddo

        call find_velo (xint,yint,zint,time,dt,fault,nfault,vx,vy,vz,xming,xmaxg,yming,ymaxg)
