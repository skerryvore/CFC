...
c
        if(lroot)open(lu_sum, file='na.sum', status='unknown')

c						write header to standard out
c	if(lroot)write(*,100)
...
c
     ...
     &        istype,monte,calcmovement,scales,misfit)
      ...
      !print*,na_models(1:nd*nsamplei)
      ! print*,na_models(1:nd)
      !print*,nd
      !print*,range(1,1:nd)
      !print*,range(2,1:nd)  
      !print*,ranget(1,1:nd) 
...
c						of header file
c
        write(tempo,'(i3)') sampleid
c       fnme = 'na.nad'
        fnme = './results/'//'na_'//trim(adjustL(tempo))//'.nad'
	call NA_header
...
c	write(*,*)
c	write(*,*)' Files created:'
c	write(*,*)
c	write(*,*)
c     &  ' na.nad : Direct access file containing ensemble of models ' 
c	write(*,*)
c     &  ' na.sum : Short summary file of progress' 
c	if(verbose)write(*,*)
c     &  ' na.det : Full details of progress' 
c	write(*,*)
...
c						number generator ran3 
              do i=1,nsample
                 do j=1,nd
                 if(time_sequence(j)) then
                    a = ran3(iseed)
                    b = 1-a
                    if(j.gt.1) then
                      if(range(1,j).gt.range(1,j-1)) then
                        low_bound=range(1,j)
                      else
                        low_bound=na_models(j-1,i)
                      endif 
                    else
                      low_bound=range(1,j)
                    endif
                    !na_models(j,i) = b*range(1,j) + a*range(2,j) 
                    na_models(j,i) = b*low_bound + a*range(2,j)          
                 else
                   a = ran3(iseed)
                   b = 1-a
                   na_models(j,i) = b*range(1,j) + a*range(2,j)
...
c	   write(50,*)' n ',n,' i',i,' j',j,' dev ',dev(i,j)
         ...
         if(time_sequence(iw)) then !Romain (timesequence)
           low_bound=x1p
           ran = ran3(iseed)
           deviate = low_bound + (x2-low_bound)*ran    
         else
           ran = ran3(iseed)
           deviate = x1 + (x2-x1)*ran
         endif
...
c						Voronoi cell of input point
              ...
              kd = id + (cell-1)*nd
              call NA_deviate (x1,x2,kd,xcur(id),iw,nd,xp)
              xp=xcur(id) ! Romain
